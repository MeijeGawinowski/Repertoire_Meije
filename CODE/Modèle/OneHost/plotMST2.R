...
filenames=paste(directory,system(paste("ls",directory),intern=TRUE),sep="")
filenames=filenames[-length(filenames)]

obsID=c()
timeObs=c()
for (ff in filenames){
  obsID=c(obsID,str_split(ff,"D")[[1]][2])
  timeObs=c(timeObs,str_split(ff,"T")[[1]][2])
}
...
for (k in 1:chain){
  # nobs=9
  # k=2
  obsID[((k-1)*nobs+1):(k*nobs)]
  # obsID=as.character(obsID[k:nobs])
  # timeObs=as.character(timeObs[k:nobs])
  # ordtime=sort(timeObs[k:nobs])
  # ordtime
  obsID2=obsID[((k-1)*nobs+1):(k*nobs)]
  timeObs2=timeObs[((k-1)*nobs+1):(k*nobs)]
  filesID=paste(directory,"D",obsID2,"DT",timeObs2,"T.fasta",sep="")
  PMST=NULL
  for(ff in filesID){
    ID=str_split(ff,"D")[[1]][2]
    time=str_split(ff,"T")[[1]][2]
    ...
    pmst <- ggplot()+theme_void()
    pmst <- pmst + geom_segment(data=edges,aes(x=X1,xend=X2,y=Y1,yend=Y2),lineend="round",size=0.4)
    pmst <- pmst + scale_y_continuous("",labels=NULL)+scale_x_continuous("",labels=NULL)
    pmst <- pmst + geom_point(aes(X1, X2, size=count, colour="red"), data=plotcord,colour="turquoise4")
    pmst <- pmst + ggtitle(paste("ID",ID,", t=",time))
    pmst
    PMST=c(PMST,list(pmst))
  }
  # do.call(grid.arrange,PMST)
  ggsave(paste("ID",ID,".pdf"),plot=do.call(grid.arrange,PMST))
}
