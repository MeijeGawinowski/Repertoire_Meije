...
runSensitivity=function(tf,par,min,max,lag){
  ...
  while (i <= max){
    ...
    if (par=="mu"){
      ...
      spar1=list(init=list(prop=1,sequ1=rep(1,1000)),mu=i,gamma=c(0.001,1000),tolerance=1e-05) 
    }
    if (par=="gamma1"){
      vpar1=list(init=list(TT=1e12,I=0,V=1e03),s=0,d=0,beta=5.0e-05,delta=2.0,p=1e-06,c=0.5,timelag=0.1,duration=tf)
      spar1=list(init=list(prop=1,sequ1=rep(1,1000)),mu=5e-06,gamma=c(i,1000),tolerance=1e-05) 
    }
    if (par=="gamma2"){
      vpar1=list(init=list(TT=1e12,I=0,V=1e03),s=0,d=0,beta=5.0e-05,delta=2.0,p=1e-06,c=0.5,timelag=0.1,duration=tf)
      spar1=list(init=list(prop=1,sequ1=rep(1,1000)),mu=5e-06,gamma=c(0.001,i),tolerance=1e-05) 
    }
    
    kin1 = viralKinetics(init=vpar1$init,vpar1$s,vpar1$d,vpar1$beta,vpar1$delta,vpar1$p,vpar1$c,vpar1$timelag,vpar1$duration)
    v_index=c(v_index,intraHostDiversity2(init=spar1$init,kinetics=kin1,mu=spar1$mu,gamma=spar1$gamma,fitnessRule=fpar2,size=100))
...
## gamma1 test for t=3
g1=runSensitivity(tf=3,par="gamma1",min=0.001,max=1,lag=0.001)
g2=runSensitivity(tf=3,par="gamma1",min=0.001,max=1,lag=0.001)
g3=runSensitivity(tf=3,par="gamma1",min=0.001,max=1,lag=0.001)
g4=runSensitivity(tf=3,par="gamma1",min=0.001,max=1,lag=0.001)
g5=runSensitivity(tf=3,par="gamma1",min=0.001,max=1,lag=0.001)
g_t=c(g1$t,g2$t,g3$t,g4$t,g5$t)
g_i=c(g1$indice,g2$indice,g3$indice,g4$indice,g5$indice)
data_g1=data.frame(g_t,g_i)
colnames(data_g1)=c("par","dist")
write.table(data_g1,"data_g1.csv",sep=",")

## gamma2 test for t=3
gg1=runSensitivity(tf=3,par="gamma2",min=1,max=10001,lag=100)
gg2=runSensitivity(tf=3,par="gamma2",min=1,max=10001,lag=100)
gg3=runSensitivity(tf=3,par="gamma2",min=1,max=10001,lag=100)
gg4=runSensitivity(tf=3,par="gamma2",min=1,max=10001,lag=100)
gg5=runSensitivity(tf=3,par="gamma2",min=1,max=10001,lag=100)
gg_t=c(gg1$t,gg2$t,gg3$t,gg4$t,gg5$t)
gg_i=c(gg1$indice,gg2$indice,gg3$indice,gg4$indice,gg5$indice)
data_g2=data.frame(gg_t,gg_i)
colnames(data_g2)=c("par","dist")
write.table(data_g2,"data_g2.csv",sep=",")
